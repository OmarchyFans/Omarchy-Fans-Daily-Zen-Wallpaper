#!/bin/bash
#
# Optional extras for Zen Wallpaper. `omarchy plugin add` already installs
...
#
#   1. symlink bin/omarchy-zen into ~/.local/bin
...
# timestamped backup is taken before a config file is appended to. Needs no root privileges.
set -euo pipefail
REPO="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
# Tools only from root-owned system folders (see bin/omarchy-zen); tests may add the plugin's own stubs.
...
export PATH
BIN="$REPO/bin/omarchy-zen"
MARK="fans.omarchy.zen-wallpaper"
YES=0; [[ ${1:-} == --yes ]] && YES=1
...
if (( ${#missing[@]} )); then
  echo "  missing: ${missing[*]} (all ship with Omarchy; install them with pacman as root)"
fi

if ask "Symlink omarchy-zen into ~/.local/bin?"; then
  mkdir -p "$HOME/.local/bin"; ln -sfn "$BIN" "$HOME/.local/bin/omarchy-zen"
  echo "  linked ~/.local/bin/omarchy-zen"
fi
...
if [[ -f $B ]] && grep -q "$MARK" "$B"; then
  ...
  echo "  SUPER + ALT + Z is already bound in $B; skipped (bind 'omarchy-zen mode still' yourself)"
elif ask "Add SUPER + ALT + Z -> toggle animated / still to $B?"; then
  [[ -f $B ]] && cp -a "$B" "$B.bak.$(date +%s)"
  cat >>"$B" <<LUA

-- Zen Wallpaper ($MARK): animated stream <-> still frame.
o.bind("SUPER + ALT + Z", "Zen wallpaper: animated / still", "$HOME/.config/omarchy/plugins/$MARK/bin/omarchy-zen toggle-mode")
LUA
