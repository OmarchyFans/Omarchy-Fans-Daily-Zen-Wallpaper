...
export PATH
"$REPO/bin/omarchy-zen" mode off >/dev/null 2>&1 || true
L="$HOME/.local/bin/omarchy-zen"
...
s = re.sub(r'\n-- Zen Wallpaper \(fans\.omarchy\.zen-wallpaper\).*?\no\.bind\("SUPER \+ ALT \+ Z".*?\n', '\n', s, flags=re.S)
open(p, "w").write(s)
PY
...
if [[ ${1:-} == --purge ]]; then
  name=$(jq -r '.theme_name // "zen"' "${XDG_CONFIG_HOME:-$HOME/.config}/omarchy-zen/config.json" 2>/dev/null || echo zen)
  [[ $name =~ ^[a-z0-9_][a-z0-9._+-]*$ ]] && rm -rf "${XDG_CONFIG_HOME:-$HOME/.config}/omarchy/themes/$name"
  rm -rf "${XDG_STATE_HOME:-$HOME/.local/state}/omarchy-zen" "${XDG_CONFIG_HOME:-$HOME/.config}/omarchy-zen" \
         "${XDG_DATA_HOME:-$HOME/.local/share}/omarchy-zen" "${XDG_CACHE_HOME:-$HOME/.cache}/omarchy-zen"
